...
func HandleError(c *gin.Context, err error) {
	traceID := c.GetString("traceId") // Optional: Set a trace TraitId per request (middleware)
	...
	case *errors.ServerError:
		logger.GetLogger().Error(e.Description,
	...
	default:
		...
		logger.GetLogger().Error("Unknown error", zap.Error(err), zap.String("traceId", traceID))
		c.JSON(http.StatusInternalServerError, gin.H{
			"error_code":        "50000",
